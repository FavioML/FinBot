...
function buildSystemPrompt() {
  ...
  return (
    ...
    + '- "Ya le pague TODO a [X]", "queda saldado TODO con [X]", "saldamos todo con [X]" = manage_debts action=settle_all (NO mark_paid). Clave: "todo" + persona = settle_all.\n'
    ...
    + '- "Lo de [comercio] es [categoria]", "El de [comercio] es [categoria]" = regla de comercio → manage_transaction action=set_category_rule (NO action=edit_store, que es para cambiar nombre).\n'
    + '- "Pon lo de [X] en [categoria]", "Mueve lo de [X] a [categoria]" refiriendose a un gasto especifico = manage_transaction action=recategorize. Usar set_category_rule solo para reglas permanentes ("siempre que vaya a X...").\n'
    ...
    + '- "Que gastos puedo eliminar para llegar a mi meta" = query_goals action=view (NO financial_query).\n'
