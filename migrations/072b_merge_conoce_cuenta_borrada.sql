-- 072b — `merge_and_link` aprende `cuenta_borrada_at`.
--
-- La regla que obliga a este archivo se pagó con la migración 059 y otra vez con la 066:
-- cuando `usuarios` gana una columna, `merge_and_link` es un CONSUMIDOR. La función BORRA
-- la fila del loser, así que **lo que no nombra, no se conserva: se destruye**. Pasó con
-- `trial_*` (B11) y con `bsuid` (B13). Lo vigila `tests/merge-and-link-columnas.test.js`,
-- que fue exactamente quien atrapó esta columna sin declarar.
--
-- Semántica: `LEAST` de las dos. `LEAST` ignora NULLs en Postgres, así que basta con que
-- UNA de las filas esté marcada para que el survivor quede marcado, conservando la fecha
-- más antigua. Mismo criterio que `trial_inicio`/`trial_vence`: el merge no regala estado
-- ni pone el reloj a cero. Partir la identidad no puede ser la forma de limpiarse una baja;
-- nada la limpia: es un HECHO. Ver el comentario de `completarAlta` en handlers/onboarding.js.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- Este archivo NO se transcribió a mano. Se generó a partir de la 066 (la vigente hasta
-- hoy) insertando UNA línea, y la equivalencia con producción está MEDIDA, no supuesta:
-- el md5 del cuerpo de la 066 —normalizado como documenta `qa-e2e/lib/usuarios-columnas.mjs`—
-- es `25008b336740cdd0f72af0906c93ab66`, idéntico al de `pg_get_functiondef` en prod una vez
-- que se le quita esta línea. O sea que la 066 sí era el espejo exacto, incluido el hotfix
-- remoto `merge_dedupe_fix_aliases` que en su momento no tuvo archivo local.
--
-- Recomprobar la paridad después de aplicar esto (el comando vive en usuarios-columnas.mjs):
--   md5 esperado del cuerpo con la línea nueva: e350ac1c4266eb9c3e65c8514f9ea8cf
