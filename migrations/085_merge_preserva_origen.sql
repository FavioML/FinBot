-- 085_merge_preserva_origen.sql
--
-- `merge_and_link` aprende `origen` y `origen_cta` (columnas de la migracion 084).
--
-- La regla que obliga a este archivo ya se pago tres veces: cuando `usuarios` gana una columna,
-- `merge_and_link` es un CONSUMIDOR. La funcion fusiona columna por columna y despues BORRA la
-- fila del loser, asi que **lo que no nombra, no se conserva: se destruye**. Paso con `trial_*`
-- (B11, migracion 059), con `bsuid` (B13, migracion 066) y con `cuenta_borrada_at` (072b). Lo
-- vigila `tests/merge-and-link-columnas.test.js`, que fue quien atrapo estas dos sin declarar.
--
-- **Aca el dano seria exactamente el bug que la 084 viene a cerrar.** El `origen` lo escribe el
-- primer mensaje de WhatsApp, o sea que vive en la fila de WhatsApp; y en el OTP inverso esa fila
-- es SIEMPRE el loser (el survivor es la cuenta de Google). Sin estas dos lineas, toda persona
-- que vincule su cuenta web pierde la atribucion de su propia alta en el momento de vincularla:
-- el canal se mediria solo para quien nunca usa la web. Gemelo exacto del caso `bsuid`.
--
-- SEMANTICA: primer toque, decidido por `created_at` y no por quien sobrevive. El par
-- (origen, origen_cta) viaja JUNTO a proposito — mezclar el canal de una fila con el boton de la
-- otra produce una fila que no describe ninguna visita real.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- Este archivo NO se transcribio a mano. Se genero a partir de la 072b (la vigente hasta hoy)
-- insertando la declaracion de `use_loser_origen`, su calculo y dos lineas en el UPDATE del
-- survivor. La equivalencia de la 072b con produccion esta MEDIDA, no supuesta: el md5 del cuerpo
-- normalizado como documenta `qa-e2e/lib/usuarios-columnas.mjs` daba
-- `e350ac1c4266eb9c3e65c8514f9ea8cf` en los dos lados el 2026-09-09, o sea que el archivo era el
-- espejo exacto de la funcion viva antes de este cambio.
--
-- Recomprobar la paridad despues de aplicar esto (el comando vive en usuarios-columnas.mjs):
--   md5 esperado del cuerpo nuevo: 65810a46642991596a10383085b6c628
...
BEGIN
  ...
  -- Primer toque: gana la fila MAS VIEJA que tenga origen, no la que sobreviva. El merge ya
  -- resuelve `created_at` con LEAST, asi que esto es la misma regla aplicada al par
  -- (origen, origen_cta) — que viaja junto, porque un canal de una fila con el boton de la otra
  -- no describe a nadie.
  use_loser_origen := (s.origen IS NULL) OR (l.origen IS NOT NULL AND l.created_at < s.created_at);
  ...
  UPDATE public.usuarios SET
    ...
    cuenta_borrada_at = LEAST(s.cuenta_borrada_at, l.cuenta_borrada_at),
    -- Migracion 084: el canal del alta sobrevive al merge. El origen lo trae la fila de
    -- WhatsApp, que en el OTP inverso es SIEMPRE el loser, asi que sin estas dos lineas el
    -- `DELETE FROM usuarios` de arriba lo destruye y el alta vuelve a quedar sin atribucion.
    -- Es el mismo mecanismo que se comio `trial_*` (B11) y `bsuid` (B13).
    origen     = CASE WHEN use_loser_origen THEN l.origen     ELSE s.origen     END,
    origen_cta = CASE WHEN use_loser_origen THEN l.origen_cta ELSE s.origen_cta END
  WHERE id = p_survivor;
