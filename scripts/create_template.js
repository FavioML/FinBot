...
async function main() {
  ...
  instrCell.value = 'PLANTILLA NETO — Carga de Gastos e Ingresos Históricos. Completa cada fila con un movimiento. Tipo, Categoría, Subcategoría, Método de Pago y Banco son opcionales (NETO las asigna automáticamente con IA).';
  instrCell.font = { size: 11, italic: true, color: { argb: 'FF666666' } };
  instrCell.alignment = { wrapText: true, vertical: 'middle' };
  ws.getRow(1).height = 40;

  // --- Headers (fila 2) ---
  const headers = ['Fecha', 'Monto', 'Comercio / Descripción', 'Tipo', 'Categoría', 'Subcategoría', 'Método de Pago', 'Banco'];
  ...
  // --- Ejemplos (filas 3-5) ---
  const ejemplos = [
    ['15/01/2026', 45.50, 'Supermercado Metro', 'Gasto', 'Alimentación', 'supermercado', 'Yape', 'BCP'],
    ['16/01/2026', 12.00, 'Taxi al centro', 'Gasto', 'Transporte', 'taxi', 'Efectivo', ''],
  ...
  // --- Validación: Subcategoría (columna F, filas 3-502) ---
  const subcategorias = [
    'delivery', 'restaurante', 'supermercado', 'mercado', 'cafeteria', 'snacks',
    'uber_cabify', 'taxi', 'bus_micro', 'metro_bus', 'gasolina', 'peaje', 'estacionamiento',
    'alquiler', 'mantenimiento', 'electricidad', 'agua', 'gas', 'internet', 'cable',
    'farmacia', 'medico', 'clinica', 'laboratorio', 'seguro_salud', 'optica',
    'streaming', 'cine', 'juegos', 'bares_clubs', 'eventos', 'hobbies',
    'ropa', 'calzado', 'electronico', 'hogar', 'belleza', 'mascotas',
    'universidad', 'instituto', 'curso_online', 'utiles', 'idiomas', 'colegios',
    'prestamo', 'tarjeta_credito', 'seguro', 'ahorro', 'inversion', 'comision_banco',
    'herramientas', 'publicidad', 'oficina', 'logistica', 'contador',
    'regalo', 'donacion', 'multa', 'viaje', 'sin_categoria'
  ];
  for (let r = 3; r <= 502; r++) {
    ws.getCell(`F${r}`).dataValidation = {
      type: 'list',
      allowBlank: true,
      formulae: ['"' + subcategorias.join(',') + '"'],
    };
  }

  // --- Validación: Método de Pago (columna G, filas 3-502) ---
  ...
  // --- Validación: Banco (columna H, filas 3-502) ---
