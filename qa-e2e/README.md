...
| `qa-atribucion-alta.mjs` | **El eslabón que convierte "sesiones por canal" en "altas por canal"**: que el `[hero|ig]` del texto prellenado de un CTA de la landing termine en `usuarios.origen`. Hasta el 2026-09-09 esa etiqueta se escribía y se descartaba, y ninguna de las 48 altas de agosto tenía canal. Cinco casos, y los tres últimos son los que le dan valor al primero: el link VIEJO sin `|origen` (queda `'directo'`, no NULL — NULL significa "alta anterior a la medición" y son dos cosas distintas), el alta YA CERRADA que no se reetiqueta (sin esa guarda, el primer clic de cualquiera de las 100+ filas viejas contamina la serie con la fecha de hoy) y el control negativo de un primer mensaje que no es un CTA (sin él, todo lo demás es compatible con un código que escribe `'directo'` en toda alta nueva). Lo que `tests/lib/atribucion.test.js` NO puede ver: que las columnas de la migración 084 existan, que su CHECK de largo acepte lo que el parser produce, y que el UPDATE pase por RLS. Siembra 4 usuarios throwaway y los borra comprobando. Fuera del canary: ESCRIBE, y sólo se rompe con un commit | Al tocar `lib/atribucion.js`, el bloque de atribución de `handlers/webhook.js`, o el formato de la etiqueta en `landing/src/lib/atribucion.ts` (son las dos mitades de un contrato entre repos) |
...
| `qa-bsuid-username.mjs` | El camino por BSUID contra producción: un usuario con username de WhatsApp llega sin `from`, se le registra el gasto y **se le contesta a su BSUID** (desde el 12-sep-2026). Lo prueba la fila `respuesta_bsuid / skipped_test` del fixture, que dice a la vez que la respuesta salió por BSUID y que no llegó a Meta. Más el mapeo pasivo por callback de status. El BSUID desconocido NO corre acá: da de alta una fila real. Lo cubre `qa-bsuid-alta.mjs`, en proceso | Al tocar `persistirBsuid`, el alta, el webhook o `lib/whatsapp.js`. Correr DESPUÉS del deploy |
| `qa-bsuid-alta.mjs` | **La mitad que el de arriba no puede correr contra producción**: alguien que oculta su número le escribe a Neto por PRIMERA vez. Webhook en proceso, Supabase real, `enviarWhatsapp` stubeado (sin `is_test_user`, el stub es la única barrera contra Meta). Tres mensajes (`hola`, el nombre, un gasto) y afirma que nace UNA fila con `whatsapp` NULL y ese BSUID —la barrera cuenta los INSERT que hace el código, así que "la creó esta corrida" es una observación y no una inferencia—, que toda respuesta va a ESE BSUID y a nadie más (el destino se resuelve con `opts`, porque `enviarWhatsapp(null, …, { usuarioId })` también entrega), que los mensajes siguientes no duplican a la persona, que el copy no pide ni muestra un número, y que no queda nada en `errores`. **Es el único harness cuya fila nace SIN la marca**, porque la escribe `altaPorBsuid` y esa forma es lo que se prueba: `qa-guard` la admite solo declarada con `esperarAltaPorBsuid`, con la forma exacta `{ whatsapp: null, bsuid }`, y la marca `is_test_user` en el mismo instante en que nace (el cuerpo sin marca queda registrado como evidencia). La borra comprobando y barre al arrancar las de corridas muertas. El copy lo juzga `lib/copy-sin-numero.mjs`, con sus evasiones fijadas en `tests/qa-e2e-copy-sin-numero.test.js`. Hasta este harness ese camino solo lo cubría un test con `resolverUsuarioEntrante` mockeado. Fuera del canary: escribe una fila sin marca y quema OpenAI | Al tocar `altaPorBsuid`, `resolverUsuarioEntrante`, el onboarding, el bloque de identidad del webhook o `enviarWhatsapp` |
| `qa-otp-intento-instrumentado.mjs` | Que el intento de vincular WhatsApp desde la webapp QUEDE MEDIDO en producción (migración 083). De esas dos columnas depende decidir si el arreglo es la pantalla de vinculación o pedir el teléfono en el alta, y una instrumentación muda es peor que ninguna: dentro de tres semanas "0 intentaron" se lee igual que "la columna no se escribe". Dos casos, y el segundo es el que un test de "escribió algo" no ve: el segundo POST **sube el contador y NO mueve la fecha** — `otp_solicitado_at` es un hecho, no un estado, y sin ese caso cambiarlo por un `new Date()` pelado pasa en verde. Pone en NULL el `whatsapp` centinela del usuario QA, mide, y RESTAURA comprobando (aborta si esa fila tuviera lo que parece un número real) | Al tocar `POST /api/onboarding` o las dos columnas de la 083 |
| `probe-envio-por-username.mjs` | **La otra mitad de la misma premisa, y la única que no depende de conseguir un teléfono ajeno**: ¿sirve el WhatsApp **username** como destinatario? Meta lo manda en `contacts[0].profile.username` de todos los mensajes sin `from` y el código lo descarta. Es un test **diferencial**: manda cada forma candidata (`to`, `recipient_type`, `username`, `recipient:{username}`, `wa_username`) contra v19 y v25, junto a un campo inventado, y compara los errores con el eco del valor borrado — porque Meta **ignora** lo que no conoce, así que un intento suelto no concluye nada. Lleva **control positivo** (`recipient_type: "individual"`, que Meta sí acepta): si ese no distingue, el probe se descarta entero en vez de reportar "no sirve". **No le llega a nadie**: el destinatario no tiene un solo dígito y Meta sanitiza `to` a dígitos; aborta si alguien le mete uno. Correr con `railway run` (el `.env` local no tiene las `META_*`). Veredicto del 03-sep-2026: **NO es direccionable** — el enum de `recipient_type` es literalmente `["group","individual",null]`. Fuera del canary: 22 POSTs por corrida y solo puede cambiar si cambia Meta | Cuando Meta anuncie envío por username, o antes de volver a asumir que a esta gente no se le puede escribir |
| `qa-otp-sin-numero.mjs` | Que el OTP inverso CIERRE sin numero: un BSUID desconocido manda `NETO-XXXXXX` y su cuenta web queda vinculada y el codigo quemado, que es la unica señal que destraba "Esperando tu confirmacion...". Tres casos, y los dos ultimos son los que le dan valor al primero: codigo inexistente (si no, el primero pasaria por "vincula siempre") y el codigo de OTRA cuenta sembrada, que tiene que vincular a ESA y no pisar la primera — el desenlace que ataria el WhatsApp de alguien a la cuenta Google de otro. Los tests unitarios no pueden verlo: afirman sobre un doble de Supabase, y lo que estuvo roto cinco semanas fue el ORDEN de dos bloques del webhook real. Verifica ademas el backfill de la migracion 081: una fila de `errores` sembrada con ese BSUID y SIN `usuario_id` queda enganchada al usuario al vincular, que es lo que la vuelve borrable por `borrar_cuenta_total`. Siembra 2 usuarios + 2 OTP + 1 error y los borra comprobando que se borraron. Afuera del canary a proposito — solo se rompe con un commit | Al tocar `services/otp-sin-numero.js`, el bloque `if (!from)` del webhook, o el OTP con numero (las ramas son espejo) |
...
| `qa-dia0-cierre.mjs` | La promesa del cierre del día contra prod, por el webhook firmado: el primer gasto que estrena la prueba avisa "esta noche a las 9 te mando el cierre" antes de las 21h. El envío del cierre lo cubren el dry-run y `notification_deliveries`. Pasa por OpenAI y escribe en prod (usuario efímero `is_test_user`) | Al tocar la promesa de `colaConfirmacionGasto` o `prometerCierreHoy`. Contra el commit anterior es el control |
| `probe-email-no-verificado.mjs` | **Mide** (solo lectura) si queda en `usuarios` algún correo que nadie probó: ni el de su usuario de auth ni el de su propio `gmail_cuentas`. Es el predicado de la migración 086. Sale exit 1 y lista las filas enmascaradas. No va al canary: sólo cambia con un commit que vuelva a guardar un correo dictado, y eso lo cortan los tests del canal (`correoVerificado`) | Antes y después de aplicar la 086, y si algún alta vuelve a pedir el correo fuera de una sesión |
| `probe-retencion-dia0.mjs` | **Mide** (solo lectura) la retención del día 0→1 por franja horaria, las pruebas terminadas y la salud del cierre del día (entregas, `131047`, bajas, `quality_rating`). Sale exit 1 si salta el freno del cierre. No es un guard: sus números dependen de tráfico real | Las lecturas del plan día 0→1 (~23-sep canal, ~6-oct retención, ~28-oct pagos), con `--desde` para la cohorte posterior al deploy |
...
| `qa-dia0-respuestas.mjs` | Las respuestas malas del día 0 contra PRODUCCIÓN, por el webhook firmado y con el clasificador real: "Por categoría", "empecemos de cero, cancela todo" y la meta ("¿cuánto por día?", viabilidad sin un mes cerrado). Usuario efímero `is_test_user` con número no asignable (`510000…`), limpieza verificada también ante Ctrl+C. Contra el commit anterior es el CONTROL. Deja como máximo 1 fila en `borrados_auditoria`. NO va al canary: pasa por OpenAI y escribe en prod | Después de desplegar un cambio al clasificador, a `crear_meta`/viabilidad o a las guardas de borrado |
...
| `qa-atribucion-web.mjs` | El alta web guarda su canal, por HTTP contra prod y encadenado: la entrada con `?utm_source` devuelve la cookie `neto_origen` saneada, el callback (magic link generado por la admin API, sin correo) crea la fila con ese `origen` + `origen_cta='web'`, y un alta sin cookie sale `'directo'`, no null. Desde el 10-sep (Acción 5) también: una invitación `/join/*` sin UTM deja la cookie `'invitacion'` y el alta nace con ese origen, y `/login` hornea `[login]` en su link de WhatsApp (4 rojos contra el deploy anterior). El pie de WhatsApp de `/join` sale NO MEDIDO: sólo existe con una invitación válida y el harness no lee las de nadie. Crea 3 usuarios de Auth y los borra, comprobando que no quede nada. Fuera del canary: se rompe CON commit | Al tocar `webapp/src/lib/atribucion.ts`, el middleware o la rama web-first de `/auth/callback` |
| `qa-login-sin-vinculo-email.mjs` | El login con Google/magic link NO adopta una fila existente por su correo (DEFECTOS 14 y 15-sep): siembra una fila "de WhatsApp" (BSUID `PE.qa…`, sin número, `is_test_user`) con el correo X, entra como X y exige que esa fila siga sin auth y que nazca un alta web SIN correo; lo mismo con X en otras mayúsculas (antes caía a `/onboarding` sin fila), y un control con correo libre que nace CON su correo. Antes del deploy de `19c5a49` dio 8 rojos: la fila adoptada y el `/onboarding`. Borra lo suyo y comprueba que no quede nada. Fuera del canary: se rompe CON commit | Al tocar `/auth/callback`, `createWebUser` o cualquier camino que vincule una sesión con una fila existente |
| `qa-atribucion-wa.mjs` | La otra mitad: que el backend DESPLEGADO convierta en `usuarios.origen` los textos de WhatsApp que se reparten hoy (`referido`, `invitacion`, `login`, `blog`) más un control sin corchete que tiene que quedar NULL. Webhook firmado a `api.neto.pe`, usuario sembrado con `is_test_user` (no le escribe a Meta) y borrado verificado. Código de referido inventado: no vincula a nadie | Al tocar el formato del corchete, una posición, o cualquier texto prellenado de WhatsApp en la landing o la webapp |
...
| `qa-soporte-feedback.mjs` | Que la QUEJA entre de verdad en `nlp_errors` (re-leida de la base, que es la unica asercion que el copy no puede fingir) y que ni ella ni el feedback repartan el celular personal de Favio. Trae sus DOS controles del detector: que VE el numero donde si esta, y que no lo inventa donde no — sin eso un detector roto pone todo en verde, que es el defecto registrado el 28-ago en docs/DEFECTOS.md. Limpia sus filas por ID (un DELETE con `like` sobre `nlp_errors` alcanzaria filas reales y el qa-guard lo rechaza); verificado que el conteo por tipo queda identico. Corrido contra prod: 11/11 | Al tocar la superficie de soporte (`social.js`, `moderacion.js`, `lib/support-tickets.js`) o el copy de contacto. **NO va al canary**: solo se rompe CON un commit, y un harness que nadie puede romper sin querer fabrica falsas alarmas |
