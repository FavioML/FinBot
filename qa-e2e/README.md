...
| `qa-otp-sin-numero.mjs` | Que el OTP inverso CIERRE sin numero: un BSUID desconocido manda `NETO-XXXXXX` y su cuenta web queda vinculada y el codigo quemado, que es la unica señal que destraba "Esperando tu confirmacion...". Tres casos, y los dos ultimos son los que le dan valor al primero: codigo inexistente (si no, el primero pasaria por "vincula siempre") y el codigo de OTRA cuenta sembrada, que tiene que vincular a ESA y no pisar la primera — el desenlace que ataria el WhatsApp de alguien a la cuenta Google de otro. Los tests unitarios no pueden verlo: afirman sobre un doble de Supabase, y lo que estuvo roto cinco semanas fue el ORDEN de dos bloques del webhook real. Siembra 2 usuarios + 2 OTP y los borra comprobando que se borraron. Afuera del canary a proposito — solo se rompe con un commit | Al tocar `services/otp-sin-numero.js`, el bloque `if (!from)` del webhook, o el OTP con numero (las ramas son espejo) |
