...
|---|---|---|
| `qa-parity-allroutes.mjs` | Barrido Free-vs-Pro sobre las 13 rutas del dashboard: errores de consola, 4xx/5xx, si aparece el `ProGate` y a dónde redirige cada una. **Hasta el 23-ago-2026 solo imprimía**: terminaba en un `JSON.stringify` sin exit code, y un React #310 que mandaba `/dashboard/presupuestos` al error boundary estuvo **once días** en su salida sin que nadie lo viera. Hoy cierra por `lib/veredicto.mjs` con dos afirmaciones por ruta y nombra la ruta que se rompió. Separa el ruido del muro con el `esperadoPorGating()` de `qa-planning-sweep.mjs` —el mismo criterio, copiado a propósito, porque dos definiciones de "qué 402 es esperado" divergen sin que nadie lo note— y tiene piso de cobertura propio: menos de 13 rutas visitadas es INCONCLUSO (exit 2) y no un OK más chico, porque el genérico de `cerrar()` dejaría pasar 12 rutas caídas y 1 sana como "2 afirmaciones verdes" | Después de tocar gating, rutas nuevas, o el layout del dashboard |
