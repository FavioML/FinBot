...
const { registrarDeuda, obtenerDeudas, abonarDeuda, marcarDeudaPagada, formatearResumenDeudas } = require('./services/debts');
...
async function intentarResolverConsulta(usuario, texto) {
  ...
  // Filtrar strings "null" que GPT puede devolver como texto literal
    ...
    switch (intencion) {
      ...
      case 'registrar_deuda': {
        try {
          ...
          if (!contraparte || !monto || monto <= 0) {
            return 'Para registrar una deuda necesito saber: ¿a quién y cuánto?\n\nEj: _"debo S/200 a Juan"_\nEj: _"Pedro me debe S/150 por la cena"_';
          }
          await registrarDeuda(usuario.id, tipo, contraparte, monto, moneda, descripcion);
          const sym = moneda === 'USD' ? '$' : 'S/';
          if (tipo === 'debo') {
            return '📤 *Deuda registrada:*\n\nLe debes a: *' + contraparte + '*\nMonto: *' + sym + ' ' + monto.toFixed(2) + '*' + (descripcion ? '\nPor: ' + descripcion : '') + '\n\n_Escribe "mis deudas" para ver el resumen._';
          } else {
            return '📥 *Deuda registrada:*\n\n*' + contraparte + '* te debe: *' + sym + ' ' + monto.toFixed(2) + '*' + (descripcion ? '\nPor: ' + descripcion : '') + '\n\n_Escribe "mis deudas" para ver el resumen._';
          }
        } catch(e) {
      ...
      case 'abonar_deuda': {
        try {
          ...
          const monto = parseFloat(datos.monto);
          if (!contraparte || !monto || monto <= 0) {
            return 'Para registrar un abono necesito saber: ¿a quién le pagaste y cuánto?\n\nEj: _"pagué 100 a Juan"_';
          }
          const resultado = await abonarDeuda(usuario.id, contraparte, monto);
          if (!resultado) {
            return 'No encontré una deuda activa con *' + contraparte + '*. Verifica el nombre con _"mis deudas"_.';
          }
          const { deuda, completada } = resultado;
          const sym = deuda.moneda === 'USD' ? '$' : 'S/';
          if (completada) {
            return '✅ *Deuda saldada!*\n\nLa deuda con *' + deuda.contraparte + '* quedó en cero. 🎉';
          }
          ...
          return '✅ *Abono registrado:*\n\n*' + deuda.contraparte + '*\nPagado: ' + sym + ' ' + (parseFloat(deuda.monto_original) - parseFloat(deuda.monto_pendiente)).toFixed(2) + ' (' + pct + '%)\nPendiente: *' + sym + ' ' + parseFloat(deuda.monto_pendiente).toFixed(2) + '*';
        } catch(e) {
      ...
      case 'marcar_deuda_pagada': {
        try {
          const contraparte = datos.contraparte;
          if (!contraparte) {
            return '¿Con quién quedó saldada la deuda?\n\nEj: _"ya pagué a Juan"_';
          }
          const deuda = await marcarDeudaPagada(usuario.id, contraparte);
          if (!deuda) {
            return 'No encontré una deuda activa con *' + contraparte + '*. Verifica con _"mis deudas"_.';
          }
          return '✅ *Deuda saldada!*\n\nLa deuda con *' + deuda.contraparte + '* (' + (deuda.moneda === 'USD' ? '$' : 'S/') + ' ' + parseFloat(deuda.monto_original).toFixed(2) + ') quedó en cero. 🎉';
        } catch(e) {
