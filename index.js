...
async function procesarMensajeLibre(msg, usuario, from) {
  try {
    ...
    if (/\b(pagu[eé]|divid[eiír]|split|repartir)\b.+\bentre\s+\d+/i.test(msg) || /\b(dividir|split|repartir)\s+(gasto|cuenta|cena|almuerzo|uber)\b/i.test(msg)) {
    ...
    switch (intencion) {
      ...
      case 'dividir_gasto_grupal': {
        try {
          // Parse "pagué 300 la cena entre 4"
          ...
          const mDesc = msg.match(/(?:pagu[eé]|divid[eiír]|split)\s+\d+[\d,.]*\s+(?:por\s+|de\s+|la\s+|el\s+)?(.+?)(?:\s+entre\s+)/i);
          const descripcion = mDesc ? mDesc[1].trim() : 'Gasto compartido';
          ...
          // Create participant placeholders
          const participantes = [];
          for (let i = 0; i < numPersonas - 1; i++) {
            participantes.push({ gasto_id: gastoComp.id, nombre: 'Persona ' + (i + 1), monto_debe: perPerson, pagado: false });
          }
          await supabase.from('gasto_participantes').insert(participantes);
          return '✅ *Gasto compartido creado*\n\n📝 ' + descripcion + '\n💰 Total: S/ ' + montoTotal.toFixed(2) + '\n👥 ' + numPersonas + ' personas\n💳 Cada uno: *S/ ' + perPerson.toFixed(2) + '*\n\n_Ve a app.neto.pe > Deudas > Compartidos para editar nombres y marcar pagos._';
        } catch(e) {
...
async function enviarAlertaTransaccion(usuario, tx, resultado) {
  ...
  // Sugerencia inteligente: si es ingreso y hay meta activa, sugerir destinar algo
  if (tipo === 'ingreso') {
    try {
...
async function generarResumenSemanal(usuario) {
  ...
  // === Deudas próximas a vencer esta semana ===
  try {
  ...
  // === Sugerencia: superávit → meta (Feature 6) ===
  try {
