...
async function recibioMensajeRecienteProactivo(userId, dias = MIN_DAYS_BETWEEN_PROACTIVE) {
  ...
  const { data, error } = await supabase.from('survey_events')
    ...
    .gte('sent_at', cutoff)
    .limit(1);
  // Falla ABIERTO, y es el gate de los SEIS triggers: sin leer el error, `data` viene null,
...
async function contarTransacciones(userId) {
  ...
  // respuesta posible: dispara `reminder_d3`/`d7` (`if (txCount > 0) return false`) contra
  // alguien que lleva meses anotando, con copy de primer gasto. (Hasta el 14-sep-2026 hacia un
  // segundo daño, elegirle el copy equivocado a `wake_up_inactive`; ese trigger se apago.)
...
async function contarTransaccionesUltimos(userId, dias) {
  ...
  const { count, error } = await supabase.from('transacciones')
    ...
    .eq('usuario_id', userId)
    .gte('fecha', cutoff);
  // Mismo `count || 0`, y los dos call-sites lo leen como permiso para mandar: `d14` corta
  // con `>= 3` y `d30` con `> 0`. El cero de un fallo pasa los dos.
  if (error) throw error;
  return count || 0;
}
 ...
 * canal caido. Hasta el 14-sep-2026 tenia un segundo consumidor, `liberarClaimSinEntrega`, que
 * se fue con los dos `wake_up` que lo llamaban (ver el bloque del apagado, mas abajo).
 *
 * Defensivo con la forma: si el chokepoint devolviera algo raro, "no salio nada" es el lado
 * seguro (no registrar), o sea reintentar.
 */
 ...
 *
 * Un one-shot tendria la misma clase al reves: reclama su unique index ANTES de enviar, asi que
 * no le alcanza con no registrar y tiene que devolver el claim. Eso hacia `liberarClaimSinEntrega`
 * para los dos `wake_up`, y se fue con ellos el 14-sep-2026. Esto registra DESPUES, asi que
 * alcanza con no registrar.
 */
async function enviarYRegistrar(usuario, eventType, mensaje) {
  ...
  return registrarEvento({
    ...
    // sobre un aviso que salio solo por la campana apaga los SEIS triggers una semana para
...
/*
 * ─── Los dos wake-up se APAGARON el 14-sep-2026 ──────────────────────────────────────────
 *
 * Aca vivian `maybeWakeUpInactive` (30+ dias desde el alta y 0 gastos en 30 dias) y, mas abajo,
 * `maybeWakeUpOnboarding` (7+ dias con el alta a medias). Los dos eran one-shot y se borraron
 * enteros, en los dos canales. El nombre queda escrito para que quien busque el trigger
 * encuentre por que no esta.
 *
 * No era cadencia ni copy: el destinatario esta definido como el que no le escribe a Neto hace
 * semanas, o sea fuera de la ventana de 24h de Meta por construccion. Medido contra produccion:
 *
 *   · `survey_wake_up_inactive`: 32 WhatsApp en 30 dias y 0 entregados (32 de 32 con 131047);
 *     56 intentos y 0 entregas en toda su historia. La campana: 41 filas y 0 leidas, con 16 de
 *     los 34 destinatarios reales sin cuenta web donde verla.
 *   · `survey_wake_up_onboarding`: 10 intentos en toda su historia, 0 entregas.
 *
 * Es el argumento que apago el recordatorio de inactividad el 01-sep (docblock de
 * `checkUpsellPro` en `cron/checks.js`). Ese apagado dejo vivo a `wake_up_inactive` porque
 * "bifurca nuevo/churn", que es un argumento sobre el copy y no sobre a quien le llega. Y ademas
 * lo solto: las filas de `inactivity_reminder` le tenian ocupada la anti-fatiga de 7 dias, y al
 * desaparecer salieron 14 de golpe el 07-sep.
 *
 * Decision de Favio, sin plantillas pagas. Los unique index (migraciones 011 y 013) y las filas
 * historicas de `survey_events` se quedan: `REMINDER_CONV_TYPES` en `cron/checks.js` las sigue
 * leyendo. Guard de comportamiento: `tests/services/wake-up-apagados.test.js`.
 */
...
async function marcarRespuestaProactiva(usuarioId, replyText) {
  try {
    ...
    // ACCESORIA, y por eso el arreglo es el opuesto al de los seis triggers: esto no decide a
...
async function checkSurveyTriggers() {
  ...
  try {
    // No filtramos por onboarding_completado aqui. El motivo escrito era `maybeWakeUpOnboarding`,
    // que necesitaba ver a los que NO completaron, y se apago el 14-sep-2026. Se deja igual a
    // proposito, porque sacar el filtro no es neutro: `reminder_d3`/`d7` exigen `txCount === 0`,
    // no el alta cerrada, asi que HOY alcanzan a quien quedo a medias. (La nota vieja decia que
    // "los demas triggers requieren completion porque dependen de tx_count > 0"; para esos dos es
    // al reves.) Filtrar aca es una decision sobre el onboarding, y no entro con el apagado.
