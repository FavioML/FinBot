...
const { anclarAnioDeCaptura } = require('../lib/dates');
...
async function extraerPagoDeImagen(buffer, mimeType, hoy) {
  ...
  // El año que el modelo escribe NO es dato leído: cuando la captura muestra "23 Ago" sin
  // año, lo inventa. La regla del prompt de arriba ayuda pero no decide — depende de que el
  // modelo obedezca, y el 24-ago-2026 no obedeció nueve veces seguidas. Esto sí decide.
  //
  // Va acá dentro y no en los llamadores a propósito: `extraerPagoDeImagen` es el único camino
  // de una captura a una transacción, lo usan el webhook y el registro silencioso, y puesto en
  // los dos se puede olvidar en uno.
  //
  // Solo REESCRIBE cuando encuentra un ancla mejor. Si no la hay (una fecha vieja de verdad,
  // un 29-feb no re-anclable) deja lo que vino: así este saneo no puede empeorar el caso de
  // quien manda a propósito el comprobante de un gasto de hace meses.
  if (parsed && parsed.fecha) {
    const anclada = anclarAnioDeCaptura(parsed.fecha, hoy);
    if (anclada && anclada !== parsed.fecha) {
      log.warn({ tag: 'IMAGEN_FECHA', crudo: parsed.fecha, anclado: anclada, hoy },
        'Vision devolvió una fecha fuera de ventana; se reancló el año');
      parsed.fecha = anclada;
    }
  }
  return parsed;
}
