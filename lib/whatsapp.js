...
async function registrarEntrega({ usuarioId, tipo, canal, estado, code, error, wamid }) {
  ...
  try {
    ...
    //
    // Importa más de lo que parece porque esta fila es el LEDGER que leen los dedup de
    // `cron/checks.js`: si no queda, el cron vuelve a considerar candidato al usuario en la
    // corrida siguiente. `checkRecordatorioOnboarding` corre cada 15 minutos sobre una ventana
    // de 3h, o sea hasta ~12 avisos al mismo usuario, y eso YA PASÓ: el 20-jul dos usuarios
    // recibieron 12 WhatsApps idénticos de tipo `onboarding` cada uno.
    //
