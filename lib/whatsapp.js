...
// backend asume instancia única, ver CLAUDE.md). Se cachea el número aunque NO exista en
// `usuarios`: reintentarlo en cada callback tampoco lo haría aparecer.
...
async function aprenderBsuidDeStatus(st) {
  ...
  try {
    ...
    const { data } = await supabase.from('usuarios').select('id, bsuid').eq('whatsapp', numero).maybeSingle();
    if (!data) return;
    await persistirBsuid(data, bsuid);   // no borra, no rompe: ver helpers/db-helpers.js
  } catch (e) {
    log.error({ tag: 'BSUID', err: e.message }, 'No se pudo aprender el BSUID desde un callback');
  }
}
