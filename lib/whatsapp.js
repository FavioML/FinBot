...
async function procesarStatuses(statuses) {
  ...
  for (const st of statuses) {
    ...
    if (!wamid) continue;
    // Sonda temporal (08-ago-2026). ¿El callback de estado trae también el BSUID? Si lo trae,
    // se puede mapear a TODOS los usuarios de forma pasiva —con cualquier notificación que ya
    // reciben— en vez de esperar a que cada uno escriba. Hoy hay 4 de 96 mapeados, y a quien
    // active un username antes de volver a escribir lo perdemos. Ver CLAUDE.md (BSUID).
    log.info({
      tag: 'BSUID-STATUS',
      claves: Object.keys(st || {}),
      recipientId: st.recipient_id || null,
      recipientUserId: st.recipient_user_id || null,
      userId: st.user_id || null,
      estado: st.status || null,
    }, 'Sonda BSUID en callback de estado');
