...
async function registrarEntrega({ usuarioId, tipo, canal, estado, code, error, wamid }) {
  ...
  try {
    ...
    // corre **cada 15 minutos** sobre una ventana de 3h: hasta ~12 avisos a la misma persona.
    //
    ...
    // se escribió es el llamador, y `enviarWhatsapp` no puede des-enviar un mensaje que Meta
    // ya aceptó. Pero un fallo visible es la diferencia entre diagnosticar esto en el log y
    // volver a deducirlo de la tabla dos meses después.
    if (deliv.error) {
