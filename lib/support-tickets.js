 ...
 *
 * @param {{ ticketId: string, rol: "usuario"|"admin", mensaje: string, patchExtra?: object }} args
 */
async function registrarMensajeTicket({ ticketId, rol, mensaje, patchExtra = {} }) {
  ...
  const { error: errHilo } = await supabase.from('tickets_mensajes').insert({
    ticket_id: ticketId, rol, mensaje: texto,
  });
  if (errHilo) {
