...
async function estadoVentana(usuarioId) {
  ...
  try {
    const [{ data: u, error: errUsuario }, { data: conv, error: errConv }] = await Promise.all([
      // `supabase_auth_id` viaja con la dirección: sin él `correoVerificado` no puede saber si la
      // dirección está probada y devuelve null, o sea que el correo se apaga en silencio.
      supabase.from('usuarios').select('email, supabase_auth_id, recordatorios_activos').eq('id', usuarioId).maybeSingle(),
    ...
    return {
      ...
      // Crudo, igual que el flag de abajo: la decisión de si la dirección está probada la toma
      // `correoVerificado` en quien manda, que es donde el guard la busca.
      supabase_auth_id: (u && u.supabase_auth_id) || null,
  ...
  try {
    ...
    if (usuarioId) {
      try {
        ...
        //
        // **Y sólo a una dirección probada** (15-sep-2026): `correoVerificado` devuelve null sin
        // cuenta web, porque ese correo lo dictó la persona por WhatsApp y nadie lo verificó.
        // Una respuesta de soporte a la bandeja equivocada le cuenta a un desconocido lo que otra
        // persona le escribió a Neto.
        const avisarPorCorreo = !ventana.abierta && ventana.recordatorios_activos !== false;
        const { notificarUsuario, CANALES } = require('./notify-user');
        const { correoVerificado } = require('./email');
        await notificarUsuario({
          ...
          email: {
            to: correoVerificado(avisarPorCorreo ? ventana : null),
 ...
 * o null si no hay. Aplica autocierre lazy: si la sesión lleva más de `SESSION_IDLE_MS`
 * sin actividad, la cierra y devuelve null (el usuario vuelve al bot).
 *
 * **`avisarAutocierre` decide si ese autocierre le avisa a la persona, y el default es sí.**
 * El aviso no es del CIERRE: es del RUTEO. Existe para el llamador de `message-processor`,
 * donde la persona no pidió nada — escribió creyendo que seguía hablando con el equipo, la
 * ventana ya venció, y su mensaje se lo lleva el bot sin que ella lo sepa. Ahí callar deja a
 * alguien preguntándole algo a soporte y recibiendo a Neto hablándole de gastos.
 *
 * El otro llamador es `abrirSesion`, y ahí el ruteo NO cambia a espaldas de nadie: la persona
 * acaba de escribir `/soporte` (o el equipo acaba de escribirle desde el panel), así que va a
 * tener soporte. Con el aviso puesto recibía dos mensajes seguidos que se leen como que el bot
 * se desmiente — *"Volví a ser tu asistente"* e inmediatamente *"Modo soporte activado"*.
 *
 * El default va en `true` a propósito: **silenciar es la dirección peligrosa**, así que un
 * llamador nuevo lo hereda avisando y sólo lo apaga quien pueda justificar que el ruteo ya
 * está anunciado por otro lado.
 *
 * @param {string} usuarioId
 * @param {{ avisarAutocierre?: boolean }} [opts]
 * @returns {Promise<object|null>} el ticket abierto, o null.
 */
async function obtenerSesionAbierta(usuarioId, { avisarAutocierre = true } = {}) {
  ...
  if (ref && (Date.now() - new Date(ref).getTime()) > SESSION_IDLE_MS) {
    // **Se avisa por DEFECTO, y no es cortesía.** Esta rama se dispara casi siempre por un mensaje
    ...
    await cerrarSesion({ usuarioId, avisarUsuario: avisarAutocierre, porInactividad: true });
...
async function abrirSesion({ usuarioId, whatsapp, nombre = null }) {
  // **El autocierre acá va SIN aviso, y es lo único que hace falta para no contradecirse.**
  // `obtenerSesionAbierta` aplica el autocierre lazy, así que quien escribía `/soporte` con una
  // sesión vencida recibía dos mensajes seguidos: *"Volví a ser tu asistente"* y, pisándolo,
  // *"Modo soporte activado"*. Los dos son ciertos por separado —se cerró la vieja, se abrió una
  // nueva— y juntos se leen como que el bot se desmiente solo.
  //
  // El aviso se apaga ACÁ y no dentro de `obtenerSesionAbierta`: allá hace falta, porque el
  // aviso no anuncia el cierre sino el RUTEO, y su otro llamador (`message-processor`) sí
  // cambia el ruteo a espaldas de alguien que no pidió nada. Los tres llamadores de esta
  // función están del otro lado de esa línea: `/soporte`, el intent `hablar_con_humano` y
  // `contactarUsuario` (donde el aviso era todavía peor: llegaba pisando la respuesta que el
  // equipo acababa de mandarle). En los tres el ruteo lo anuncia el mensaje que sigue.
  //
  // Lo que NO cambia: la sesión vencida se cierra igual. Silenciar el aviso no es saltarse el
  // autocierre — si la vieja quedara abierta, `abrirSesion` devolvería `yaAbierta: true` sobre
  // una sesión muerta y `/soporte` contestaría "ya estás en modo soporte".
  const existente = await obtenerSesionAbierta(usuarioId, { avisarAutocierre: false });
