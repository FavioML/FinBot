...
async function contactarUsuario({ usuarioId = null, whatsapp, nombre = null, mensaje, abrirConversacion = false }) {
  ...
  if (ticket && ticket.id) {
    // Sin `estado`: la sesión recién abierta se queda en `esperando_mensaje` a propósito —
    // marcarla 'respondido' diría que se contestó algo que nadie preguntó por este canal.
    //
    // Sin `wamid`: acá el envío lo hizo `responderTicket`, que no lo devuelve. Es una pérdida
    // DECLARADA y no un olvido — este turno se va a ver como "sin dato de entrega" en el
    // panel, que es la verdad. Propagarlo obligaría a cambiar la firma de `responderTicket`,
    // que hoy devuelve exactamente lo que el admin necesita leer.
    await registrarMensajeTicket({ ticketId: ticket.id, rol: 'admin', mensaje });
  }
