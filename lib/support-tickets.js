...
async function estadoVentana(usuarioId) {
  if (!usuarioId) return { abierta: true, email: null };
  try {
    const [{ data: u }, { data: conv }] = await Promise.all([
    ...
    return { abierta: true, email: null, recordatorios_activos: true };
  }
}
...
async function responderTicket({ numDestino = null, mensaje, ticketId = null, usuarioId: usuarioIdArg = null }) {
  ...
  try {
    ...
    return { ok: true, msg: '✅ Respuesta enviada a ' + numero + '.', wamid: envio.msgId || null };
  } catch (e) {
...
async function cerrarSesion({ usuarioId = null, whatsapp = null, avisarUsuario = false, porInactividad = false }) {
  ...
  if (avisarUsuario && numAviso) {
    ...
    try {
      await enviarWhatsapp(numAviso, aviso);
    } catch (e) { /* best-effort */ }
  }
