...
async function notificarUsuario({
  ...
  usuarioId = null,
  whatsapp = null,
  // La dirección de respaldo cuando no hay número: `enviarWhatsapp` la usa solo si `whatsapp`
  // viene vacío (12-sep-2026). La pasa el llamador, igual que `whatsapp` y `email.to`: leerla
  // acá metería I/O en la única función que hoy no lo necesita (ver el corte revertido abajo).
  bsuid = null,
  tipo = null,
  mensaje = '',
  template = null,
  ...
  async function mandarWhatsapp() {
    ...
    try {
      ...
      wa = await enviarWhatsapp(whatsapp, mensaje, { tipo, usuarioId, template, bsuid });
    } catch (e) {
