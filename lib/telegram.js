...
/**
 * Envía una foto (por URL) al admin con botones inline debajo.
 *
 * Se usa para la solicitud de Pro desde la webapp: el admin ve la captura del
 * comprobante y aprueba/rechaza de un tap, sin teclear comandos. Los botones inline
 * son exclusivos de Telegram (WhatsApp no los soporta), por eso este path no cae a
 * WhatsApp; el caller decide el fallback.
 *
 * @param {string|number} chatId
 * @param {string} fotoUrl - URL pública/firmada de la imagen (Telegram la descarga).
 ...
 */
async function enviarTelegramFotoConBotones(chatId, fotoUrl, caption, inlineKeyboard) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token || !chatId || !fotoUrl) return { ok: false };
  try {
