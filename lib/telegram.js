...
async function responderCallback(callbackQueryId, text) {
  ...
  try {
    // **Telegram rechaza con HTTP 200 y `ok:false` en el cuerpo.** Devolver `true` sin mirarlo
    // decía "el admin ya leyó esto" sobre una respuesta que Telegram descartó, y lo que se
    // descarta es justo el diagnóstico que 9A puso acá: "no se activó Pro y la solicitud NO
    // volvió a pendiente", "NO uses /activar (registra S/0)". El rechazo más común no es
    // exótico —un callback de más de ~15 min ya está vencido—, así que el `false` es el que
    // le permite al webhook mandar el diagnóstico por el otro canal. Es el mismo chequeo que
    // `enviarFotoConBotones` hace veinte líneas más arriba.
    ...
    log.error({ tag: 'TELEGRAM', status: res.status, err: data && data.description }, 'Telegram rechazó el answerCallbackQuery');
    return false;
  } catch (e) {
