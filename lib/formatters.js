...
function formatearPendientes(consultas) {
  ...
  var items = consultas.map(function(c, i) {
    ...
    var sym = (c.moneda === 'USD') ? '$' : 'S/';
    return (i+1) + '. *' + (c.banco||'Pago') + '* ' + sym + ' ' + parseFloat(c.monto||0).toFixed(2) + ' (' + (c.fecha||'') + ') -- ' + (ms<3600000?'hace menos de 1h':horas<24?horas+'h atras':Math.round(horas/24)+'d atras');
  });
