...
const { hoyPeru, sumarDias, ahoraPeru } = require('./dates');
...
/**
 * Las columnas que `mensajeMuro` necesita para NO elegir una rama por accidente.
 *
 * Existe porque este select estaba copiado a mano en los llamadores y ya divergió DOS veces,
 * las dos con daño y sin que ningún test se pusiera rojo:
 *   · 2026-08-01 — faltaba `trial_estado`, y el mensaje del día 15 le prometía otros 14 días
 *     gratis a quien acababa de gastarlos;
 *   · 2026-09-05 — faltaba `supabase_auth_id`, y el link de Pro se iba por el canal
 *     equivocado (token inerte al que tiene cuenta web, /login al que no la tiene).
 *
 * La segunda la encontró una revisión adversarial midiendo, no la suite: quitarle la columna
 * al select del cron dejaba los 3165 tests en verde. Un guard que compare cadenas tendría el
 * mismo problema que el select copiado — así que no se vigila la copia, se elimina: el
 * llamador CONCATENA lo suyo a esta constante en vez de reescribirla.
 */
const COLUMNAS_MURO = 'id, nombre, plan, trial_estado, trial_vence, premium_desde, premium_vence, supabase_auth_id';
...
// El cierre del día de la prueba (`cron/checks.js:checkCierreDiaPrueba`). Viven acá porque la
// promesa del primer gasto ("esta noche a las 9…", abajo en `colaConfirmacionGasto`) y el cron
// tienen que decir lo mismo: si uno cambia de hora o de días y el otro no, se promete algo que
// no sale.
const HORA_CIERRE_PRUEBA = 21;
const DIAS_CIERRE_PRUEBA = [0, 1, 2];

/**
 * Qué día de la prueba es hoy: 0 = el del primer gasto. En días de CALENDARIO de Lima, no en
 * bloques de 24h: quien anotó a las 23:50 está en su día 1 diez minutos después, y es lo que
 * la persona entiende por "mañana".
 *
 * `trial_inicio` es un timestamptz: la fecha se saca en la zona de Lima y no con `slice(0,10)`,
 * que daría la fecha UTC (un primer gasto a las 20h Lima ya es "mañana" en UTC).
 *
 * @returns {number|null}  null si no está en prueba o no hay `trial_inicio` legible
 */
function diaDePrueba(usuario, hoyStr) {
  if (!enTrial(usuario) || !usuario.trial_inicio) return null;
  const t = new Date(usuario.trial_inicio);
  if (isNaN(t.getTime())) return null;
  const inicio = t.toLocaleDateString('en-CA', { timeZone: 'America/Lima' });
  const hoy = hoyStr || hoyPeru();
  const ms = new Date(hoy + 'T12:00:00-05:00').getTime() - new Date(inicio + 'T12:00:00-05:00').getTime();
  const n = Math.round(ms / 86400000);
  return n >= 0 ? n : null;
}

/**
 * ¿Se le puede prometer al primer gasto que esta noche sale su cierre? Exige lo mismo que el cron,
 * para que la promesa no quede colgada:
 *   · antes de las 21h (después ya no hay cierre hoy);
 *   · es un GASTO y es de HOY — el cierre resume los gastos con fecha de hoy: un "ayer gasté 20"
 *     o un primer movimiento que es un ingreso ("me pagaron 1500") no aparecerían, y el cron no
 *     mandaría nada (revisión adversarial, 14-sep);
 *   · no se silenció ni tiene Manos Libres (el cron excluye a los dos);
 *   · el freno no está puesto.
 */
function prometerCierreHoy(usuario, tx, ahora) {
  if (process.env.CIERRE_DIA_PRUEBA === 'off') return false;
  if (!usuario || usuario.recordatorios_activos === false || usuario.manos_libres === true) return false;
  if (!tx || tx.tipo !== 'gasto' || String(tx.fecha || '').slice(0, 10) !== hoyPeru()) return false;
  const h = ahora || ahoraPeru();
  return h.getHours() < HORA_CIERRE_PRUEBA;
}
...
/**
 * El link a Pro que se manda POR WHATSAPP. Único dueño.
 *
 * `linkPanelPro` ya bifurcaba por identidad, y su docblock nombra el daño exacto: al
 * WhatsApp-only, `/dashboard/pro` lo deposita en `/login`, donde un "Continuar con Google"
 * le crea una cuenta HUÉRFANA en vez de vincularse a su número. Pero cuatro mensajes de
 * WhatsApp hardcodeaban esa URL y nunca lo llamaban: `pieMuro`, `nudgeMuro`,
 * `mensajeGmailProPagado` y el rechazo de pago. Medido el 05-sep-2026: **47 usuarios en el
 * muro son WhatsApp-only** y 11 de ellos tienen gastos, o sea que reciben `nudgeMuro`
 * pegado a CADA confirmación.
 *
 * El fallback a la URL pelada se conserva a propósito: sin `ACTIVATION_TOKEN_SECRET`,
 * `linkPanelPro` devuelve null y es preferible dar una dirección que no dar ninguna. Es el
 * mismo criterio de `mensajeConectarEnLaApp`.
 */
function enlacePro(usuario) {
  // Una fila SIN la columna (select acotado) no es lo mismo que una fila con la columna en
  // null, y `linkPanelPro` no las distingue: leería `undefined` y mandaría por el camino de
  // activación a alguien que SÍ tiene cuenta web, con un token inerte que termina en /login.
  // Es la misma clase que el `trial_estado === undefined` de `mensajeMuro`, unas líneas más
  // abajo, y le pasaría hoy a `checkTrialExpiry` si alguien le saca la columna del select.
  // No se adivina: se avisa y se cae al comportamiento anterior, que es el que ya tenían
  // todos estos mensajes.
  // Se compara el VALOR y no la presencia de la clave. `'x' in obj` da true para una clave
  // puesta explicitamente en `undefined`, que es justo el valor que `linkPanelPro` lee mal, y
  // una proyeccion `{ id: u.id, supabase_auth_id: u.supabase_auth_id }` sobre una fila cuyo
  // select perdio la columna produce exactamente eso. Con `in`, detector mudo y suite verde:
  // medido por una revision adversarial el 05-sep-2026, que mostro al usuario CON cuenta web
  // recibiendo el token inerte. `=== undefined` cubre las dos formas y sigue distinguiendo
  // `null`, que es un dato legitimo (usuario WhatsApp-only).
  if (usuario && usuario.supabase_auth_id === undefined) {
    log.error({ tag: 'TRIAL', usuarioId: usuario.id },
      'enlacePro recibió una fila sin supabase_auth_id: el select del llamador está incompleto y el link puede ir al canal equivocado');
    return WEBAPP_URL + '/dashboard/pro';
  }
  return linkPanelPro(usuario) || WEBAPP_URL + '/dashboard/pro';
}

/**
 * Qué se le contesta a quien pide "el dashboard" desde WhatsApp. Único dueño.
 *
 * Este texto vivía IDÉNTICO en dos archivos (`handlers/intents/reportes.js` y la cascada de
 * `/dashboard` y `/app` en `handlers/webhook.js`) y decía siempre lo mismo: *"Ahí puedes ver
 * gráficos, metas, reportes PDF, suscripciones y más"*. Para alguien en el muro eso es falso,
 * y no es un caso raro: `ver_dashboard` está en `INTENTS_LIBRES` y `/dashboard` no está en
 * `COMANDOS_LECTURA`, o sea que el muro NO los corta — a propósito, porque son superficie de
 * conversión. Medido el 05-sep-2026: **124 usuarios están en el muro**, de 152.
 *
 * Lo que recibía era una promesa que la webapp contradice dos clics después. El usuario no ve
 * un error crudo (`requireLectura` devuelve 402 y la UI monta el `Paywall`), pero descubre
 * ahí que lo que le prometieron era de pago, después de haber hecho login. Prometer y que la
 * pantalla siguiente desmienta es peor que decirlo de entrada, y además desperdicia el
 * momento: acá es donde se puede vender.
 *
 * Se conserva la regla que no se negocia — anotar es gratis para siempre — porque sin esa
 * línea el mensaje se lee como "Neto dejó de funcionar".
 */
function mensajeDashboard(usuario) {
  ...
  return '📊 ' + (pn ? pn + ', tu' : 'Tu') + ' dashboard está en:\n\n🔗 ' + enlacePro(usuario) + '\n\n' +
    // NO se promete 'ahí ves cuánto llevas gastado': ese número lo muestra el Paywall, que se
    // monta en /dashboard, y enlacePro apunta a /dashboard/pro, que no lo tiene. Lo señaló la
    // revisión adversarial del 05-sep-2026 — la misma clase que esta función dice arreglar.
    'Los gráficos, el historial y los reportes son de *Neto Pro*, así que hoy te va a pedir activarlo.\n\n' +
    'Sigo anotando todo lo que me mandes, eso no se cobra nunca.';
}

/**
 * El link a una pantalla CONCRETA de la app, respetando la identidad. Devuelve
 * `{ url, requiereActivacion }` porque el copy tiene que cambiar según la rama.
 *
 * `enlacePro` no sirve para esto: siempre apunta a `/dashboard/pro`. Y mandar una ruta
 * profunda (`/dashboard/espacios`) a un usuario sin cuenta web es el MISMO bug que este
 * módulo existe para evitar — `webapp/middleware.ts` redirige cualquier `/dashboard/*` sin
 * sesión a `/login`, donde "Continuar con Google" le crea la cuenta huérfana. Lo señaló una
 * revisión adversarial el 05-sep-2026 sobre el commit que arreglaba exactamente eso en otros
 * cuatro sitios: la clase se reintrodujo en el mismo diff que la cerraba.
 *
 * Al WhatsApp-only se le da el link de ACTIVACIÓN, que aterriza en `/dashboard`, y el copy le
 * dice que después navegue. No se le promete la pantalla directa porque no la va a ver.
 */
function enlaceApp(usuario, ruta) {
  const directa = WEBAPP_URL + ruta;
  if (usuario && usuario.supabase_auth_id === undefined) {
    log.error({ tag: 'TRIAL', usuarioId: usuario.id },
      'enlaceApp recibió una fila sin supabase_auth_id: el select del llamador está incompleto y el link puede ir al canal equivocado');
    return { url: directa, requiereActivacion: false };
  }
  if (usuario && usuario.supabase_auth_id) return { url: directa, requiereActivacion: false };
  const activacion = construirLinkActivacion(usuario ? usuario.id : null);
  // Sin `ACTIVATION_TOKEN_SECRET` no se inventa una URL: se da la directa, que es lo que había
  // antes. Mismo criterio que `mensajeConectarEnLaApp`.
  return activacion ? { url: activacion, requiereActivacion: true } : { url: directa, requiereActivacion: false };
}

/** Pie común de los tres mensajes del muro: qué recuperas y cuánto cuesta. */
function pieMuro(cuantos, verbo, usuario) {
...
/**
 * El cartel del muro. **No decide nada**: para cuando se llama, el llamador ya resolvió con
 * `estaEnMuro(usuario)` que esta persona no pasa.
 *
 * @param {object|null} usuario  Fila de `usuarios`. `trial_estado` **tiene que venir en el
 *   select**: `undefined` ahí es una fila parcial (bug del llamador) y se loguea, no se
 *   interpreta.
 * @param {number|null|undefined} conteoTx  Transacciones del usuario. Los tres valores dicen
 *   cosas distintas y el mensaje cambia con cada uno:
 *     · un número  → se recita ("*40 gastos*", "tu próximo gasto")
 *     · `0`/`null` → de verdad no anotó nada ("tu primer gasto")
 *     · `undefined` → **no se pudo contar**: el llamador leyó un `{ error }`. Degrada a la
 *       forma neutra, que es verdadera en los tres casos.
 *   Quien lea un error y pase `null` reintroduce el defecto del 24-ago
 *   (`handlers/muro-gate.js`, ver `docs/DEFECTOS.md`): le dice "tu primer gasto" a alguien
 *   con decenas. `count` de PostgREST vuelve `null` al fallar, así que la traducción a
 *   `undefined` es responsabilidad del call-site, que es el único que sabe si hubo error.
 */
function mensajeMuro(usuario, conteoTx) {
  ...
  if (usuario && usuario.trial_estado === undefined) {
    ...
    return saludo('Para') + ' ver tus gastos en gráficos necesitas *Neto Pro*.\n\n' +
      pieMuro(cuantos, 'verlos', usuario);
  }

  if (!usuario || !usuario.trial_estado) {
    // `undefined` es "no se pudo contar" y NO es cero: mismo criterio que el
    // `trial_estado === undefined` de arriba. Lo pasan así los dos llamadores que leen su
    // `{ error }` (`handlers/muro-gate.js`, `handlers/webhook.js`), porque con la lectura
    // caída `count` vuelve null, y null acá elegía "primer". Medido el 31-ago-2026: hay
    // **19 usuarios reales free con `trial_estado` NULL y transacciones, máximo 63**, o sea
    // que un hipo del conteo le afirmaba a alguien con 63 gastos que le falta el primero.
    // La forma neutra es verdadera en los tres casos. Anotado en `docs/DEFECTOS.md`.
    const cuandoSeActivan = conteoTx === undefined
      ? 'se activan cuando registres un gasto'
      : 'se activan cuando registres tu ' + (conteoTx ? 'próximo' : 'primer') + ' gasto';
    ...
    return saludo('Para') + ' ver tus gastos en gráficos necesitas *Neto Pro*.\n\n' +
      'Y tienes *' + TRIAL_DIAS + ' días gratis* esperándote: ' + cuandoSeActivan + '.\n\n' +
  ...
  if (usuario.trial_estado === 'convertido' && (usuario.premium_desde || usuario.premium_vence)) {
    ...
    return saludo('Tu') + ' *Neto Pro*' + (vencio ? ' venció el ' + vencio : ' venció') + '.\n\n' +
      pieMuro(cuantos, 'volver a verlos', usuario);
  }
...
async function nudgeMuro(usuario) {
  ...
  return '\n\nVan *S/' + total.toFixed(2) + '* este mes.\n' +
    '👉 Mira en qué se te fue: ' + enlacePro(usuario);
}
 ...
 * @param {object} [opts]
 * @param {boolean} [opts.prometeCierre=true]  false donde el mensaje entrante NO deja turno en
 *   `conversaciones` (la foto): el cierre de la noche solo sale a quien escribió hoy, así que
 *   prometérselo a quien solo mandó una captura sería prometer algo que no llega.
 * @returns {Promise<string|null>}
 */
async function colaConfirmacionGasto(usuario, tx, conteoTx, { prometeCierre = true } = {}) {
  if (!usuario) return null;
  ...
  if (tx && tx.trialIniciado) {
    ...
    // El cierre de la noche llega esperado y no como spam (plan día 0→1, paso 2). Va antes del
    // link porque el link es la acción y esto es contexto.
    const cierre = prometeCierre && prometerCierreHoy(usuario, tx)
      ? '\n\n🌙 Esta noche a las 9 te mando el cierre de tu día.'
      : '';
