...
function mensajeMuro(usuario, conteoTx) {
  ...
  if (!usuario || !usuario.trial_estado) {
    return '🔒 ' + (pn ? pn + ', p' : 'P') + 'ara ver tus gastos en gráficos necesitas *Neto Pro*.\n\n' +
      'Y tienes *' + TRIAL_DIAS + ' días gratis* sin tarjeta esperándote: se activan solos con tu próximo gasto.\n\n' +
      '📝 Anótame uno: _"gasté 20 en taxi"_';
  }

  const cuando = usuario.trial_vence ? String(usuario.trial_vence).slice(0, 10) : null;
