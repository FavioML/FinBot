 ...
 */
function mensajeGmailProPagado(usuario) {
  const pn = usuario && usuario.nombre ? usuario.nombre.split(' ')[0] : null;
  const precio = '💰 *S/' + PRO_PRECIOS.mensual + '/mes* o *S/' + PRO_PRECIOS.anual + '/año*\n' +
    '👉 ' + WEBAPP_URL + '/dashboard/pro';

  if (enTrial(usuario)) {
    return '📧 ' + (pn ? pn + ', leer' : 'Leer') + ' tus correos bancarios se activa con *Pro pagado*.\n\n' +
      'Tu prueba te da todo lo demás. Esta es la única que pide el pago primero: ' +
      'conectar tu Gmail nos cuesta un cupo con Google y los tenemos contados.\n\n' +
      'Actívalo y empiezo a leer tus correos el mismo día:\n' + precio;
  }

  return '📧 ' + (pn ? pn + ', leer' : 'Leer') + ' tus correos bancarios es parte de *Neto Pro*.\n\n' +
