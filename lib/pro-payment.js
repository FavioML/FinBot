...
const { linkPanelPro, enlacePro } = require('./trial');
...
const { lineasIdentidad, contarPagosConPlata, lineaHistorialAntes } = require('./admin-ficha');
...
async function notificarSolicitudAdminPro({ pagoId, usuario, from, montoDet, tipoPlan, comprobanteBuffer, mimeType, origen }) {
  ...
  // En paralelo con el conteo de pagos previos (más abajo, en la ficha): son dos lecturas que
  // van delante de la foto, y en serie el admin espera las dos.
  const [infoRef, previos] = await Promise.all([
    resumenReferidoParaAdmin(usuario.id),
    contarPagosConPlata(usuario.id, { excluirPagoId: pagoId }),
  ]);
  let refLines = '';
  if (infoRef.parcial) {
    // No adivina: dice que no sabe. El default de este resumen (0% y sin referrer) es
    // idéntico al de un usuario sin referido, así que sin esta línea el admin aprobaría
    // creyendo que se esperan S/10 cuando quizá se esperan S/5.
    refLines += '⚠️ No pude leer el contexto de referido (descuento / referrer). Reintenta antes de aprobar.\n';
  }
  ...
  // Quién es y si ya pagó antes: con eso el admin sabe si es cliente nuevo o recurrente sin
  // salir del chat. `from` sigue abajo porque `/pago` necesita los dígitos crudos.
  const fichaCliente = lineasIdentidad(usuario).join('\n') + '\n' + lineaHistorialAntes(previos) + '\n';
  ...
  await notificarAdmin(
    ...
    // `/pago <num>` busca por teléfono: a quien escribe sin mostrar su número (llega por BSUID)
    // no lo encontraría, y el comando le diría al admin que el usuario no existe. Se pregunta si
    // son DÍGITOS, que es lo que el comando necesita, y no si es un BSUID.
    (!/^\d+$/.test(String(from || ''))
      ? '\nApruébalo en el admin (app.neto.pe/admin/operacion) o con los botones: escribe sin número visible, así que /pago no lo encuentra.'
 ...
 * necesita saber si quedó a medias. (Lo pedía el camino silencioso del usuario sin número, que se
 * retiró el 12-sep-2026 al poder contestarle por BSUID; la forma de tres resultados se queda.)
...
async function registrarSolicitudPro({ usuario, monto, montoDetectado, tipoPlan, metodoPago, comprobanteBuffer, mimeType, origen, yaReclamado = false }) {
  ...
  await notificarSolicitudAdminPro({
    pagoId,
    usuario,
    // La dirección, no solo el número: quien escribe sin número visible tiene solo el BSUID.
    from: usuario.whatsapp || usuario.bsuid || null,
...
async function rechazarSolicitudPro({ pagoId, usuario, motivo }) {
  ...
  const mensaje = claimLimpio
    ? '⚠️ *No pudimos validar tu pago Pro*\n\n' +
      (motivo ? motivo + '\n\n' : '') +
      'Si ya yapeaste S/' + PRO_PRECIOS.mensual + ' (mensual) o S/' + PRO_PRECIOS.anual + ' (anual) a *Favio Mendoza* (970398192), reenvíanos la captura correcta desde ' + enlacePro(usuario) + ' o por aquí. 📸'
    : '⚠️ *No pudimos validar tu pago Pro*\n\n' +
      (motivo ? motivo + '\n\n' : '') +
