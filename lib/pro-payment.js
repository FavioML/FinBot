...
async function registrarPagoAprobado(usuarioId, { tipoPlan, monto, premiumDesde, premiumVence, aprobadoPor, pagoId }) {
  ...
  try {
    if (pagoId) {
      ...
      //
      // El `monto` de la fila reclamada MANDA sobre el precio de lista. `activarPro` llama
      // acá con `monto: undefined` en toda conversión pagada, así que el fallback pisaba lo
      // que la fila ya decía: una solicitud con el 50% off de referido entraba a la cola en
      // S/5 (`routes/pro.js` la escribe con `precioProEfectivo`) y al aprobarla se guardaba
      // S/10. `cajaDelMes` (webapp/src/lib/admin-revenue.ts) suma esta columna, o sea que
      // cada conversión con descuento inflaba la caja del mes en S/5 de plata que nadie
      // transfirió — la otra mitad de B10, encontrada por el revisor del diff de la ola 4.
      //
      // Solo se preserva si la fila trae un monto: un `null` (el camino de WhatsApp, donde
      // el monto sale de leer la imagen y puede no detectarse) sigue cayendo al de lista.
      // Y un `monto` explícito del llamador gana siempre — es como el comp escribe su S/0.
      let montoFila = montoFinal;
      if (monto == null) {
        const { data: filaPrevia, error: errLee } = await supabase.from('pagos')
          .select('monto').eq('id', pagoId).maybeSingle();
        if (errLee) {
