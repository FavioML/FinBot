 ...
 *   (webapp /aprobar-pago, /pago, callback Telegram). Dispara el premio de referidos al
 *   referrer. Debe quedar false en comps (/activar): un comp no puede premiar (anti-cadena).
 * @returns {Promise<{venceStr:string, mensaje:string}>}
 */
async function activarPro({ usuario, tipoPlan, aprobadoPor, enviarOAuth = true, guardarHistorial = true, resetOnboarding = false, pagoId = null, esConversionPagada = false }) {
