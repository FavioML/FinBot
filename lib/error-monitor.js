  ...
  try {
    ...
    if (entry.count >= PATTERN_THRESHOLD && (ahora - entry.lastNotified > ALERT_COOLDOWN_MS)) {
      ...
      const alertMsg = esSinNumeroVisible(mensaje)
        // Desde el 12-sep-2026 esta fila ya NO la produce quien oculta su número: esa persona
        // llega con BSUID y se atiende por el camino normal (se le contesta por BSUID). Lo único
        // que cae acá es un mensaje que Meta mandó sin NINGUNA identidad, como los 4 del 01-ago.
        // Esta alerta decía "lo que no funciona es contestarles", y dejó de ser cierto.
        ? '📵 *MENSAJES SIN REMITENTE*\n\n' +
          entry.count + ' mensajes en la última hora llegaron sin número NI BSUID, así que no hay ' +
          'a quién contestarles ni con quién asociarlos.\n\n' +
          'No es gente con el número oculto: a esa Neto le contesta por BSUID. Esto es Meta ' +
          'mandando un payload sin identidad (pasó el 01-ago-2026). Si se repite, mirar la forma.\n\n' +
          '👉 Qué mirar: `select detalle from errores where mensaje ilike \'%sin from%\' ' +
          'order by created_at desc` — trae las claves del payload.\n\n' +
