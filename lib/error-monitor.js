...
async function registrarError(tag, mensaje, opts = {}) {
  try {
    ...
    //
    // **`actor` NO se persiste, a propósito y con su límite dicho:** vive sólo en este `Map` y
    // muere con el deploy, así que una alerta que dijo "5 personas distintas" no se puede
    // reconstruir después *desde acá*. Se reconstruye desde el `detalle` de las filas, que es
    // donde el llamador ya guarda el identificador. Si algún día hace falta el agregado
    // histórico, la columna va en `errores` — no asumas que este parámetro la escribe.
    if (opts.actor) entry.actores.add(String(opts.actor));
