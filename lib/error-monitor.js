...
/**
 * ¿Este error es "llegó alguien con el número oculto y no lo podemos atender"?
 *
 * El copy genérico rotula esto como *"puede indicar un problema sistémico"* y las dos mitades
 * están mal: no es un problema del sistema —Neto funciona— y "puede indicar" manda a buscar un
 * bug que no existe. Lo que pasa es que Meta dejó de mandar el número de esa persona, y lo que
 * hay que mirar no es un stack sino CUÁNTA GENTE distinta cayó ahí.
 *
 * Por eso esta alerta cuenta personas y no mensajes: nueve avisos de UNA persona insistiendo es
 * un caso de soporte, y cinco personas en una hora es el canal de adquisición perdiendo gente.
 * El copy genérico los mostraba idénticos, porque los dos son "5+ veces en la última hora".
 */
const esSinNumeroVisible = (mensaje) =>
  /mensaje entrante sin from/i.test(String(mensaje || ''));
...
async function registrarError(tag, mensaje, opts = {}) {
  try {
    ...
    const { error: errInsert } = await supabase.from('errores').insert({
      ...
      // **Quién lo provocó cuando NO hay número ni usuario todavía** (migración 081). No es
      // diagnóstico: es lo que hace que esta fila se pueda BORRAR el día que esa persona se
      // vincule y pida la baja — `services/otp-sin-numero.js` rellena `usuario_id` buscando por
      // acá, y de ahí en más la alcanza el barrido que `borrar_cuenta_total` ya hace.
      bsuid: opts.bsuid || null,
    ...
    if (!_errorCounts[key]) {
      _errorCounts[key] = { count: 0, firstSeen: ahora, lastNotified: 0, actores: new Set() };
    }
    ...
    entry.count++;
    // Quién lo provocó, cuando el llamador lo sabe. **El conteo de personas distintas es lo que
    // separa dos incidentes que el contador de mensajes muestra idénticos**: una persona
    // reintentando nueve veces y nueve personas distintas cayendo una vez cada una disparan el
    // mismo umbral y significan cosas opuestas. Sin esto la alerta obliga a ir a la base para
    // saber cuál de las dos es.
    //
    // El mismo `bsuid` que se acaba de guardar en la fila. La ventana en memoria es la que decide
    // el copy de la alerta; el agregado histórico sale de la columna (migración 081), así que una
    // alerta que dijo "5 personas distintas" **sí** se puede reconstruir después de un deploy:
    //   select count(distinct bsuid) from errores where mensaje ilike '%sin from%' and ...
    if (opts.bsuid) entry.actores.add(String(opts.bsuid));
    ...
    if (entry.count >= PATTERN_THRESHOLD && (ahora - entry.lastNotified > ALERT_COOLDOWN_MS)) {
      ...
      // Cuántas personas distintas, cuando se sabe. `entry.actores` sólo se puebla si el
      // llamador manda `actor`, así que un cero acá es "no me lo dijeron", no "nadie".
      const personas = entry.actores.size;
      const alertMsg = esSinNumeroVisible(mensaje)
        ? '📵 *USUARIOS SIN NÚMERO VISIBLE*\n\n' +
          (personas === 1
            ? '*Una persona* le escribió a Neto ' + entry.count + ' veces en la última hora y no ' +
              'recibió nada. Está insistiendo.\n\n'
            : personas > 1
              ? '*' + personas + ' personas distintas* le escribieron a Neto en la última hora ' +
                '(' + entry.count + ' mensajes) y ninguna recibió nada.\n\n'
              : entry.count + ' mensajes en la última hora sin poder atenderse.\n\n') +
          'Llegan con el número oculto (WhatsApp Usernames + BSUID), así que no hay a quién ' +
          'responderle. *Neto no está roto*: es el rollout de Meta.\n\n' +
          '✅ Si mandan su código NETO-XXXXXX, su cuenta web SÍ se vincula sola y el registro ' +
          'de gastos les funciona en silencio.\n' +
          '❌ Lo que no funciona es contestarles. Eso depende de que Meta habilite el envío ' +
          'por BSUID.\n\n' +
          '👉 Qué mirar: `select detalle from errores where mensaje ilike \'%sin from%\' ' +
          'order by created_at desc` — ahora trae el texto de lo que mandaron.\n\n' +
          '🔄 *' + entry.count + ' mensajes* en la última hora\n' +
          '_' + fecha + '_'
        : esOpenAISinCreditos(mensaje)
        ? '🔴 *OPENAI SIN CRÉDITOS*\n\n' +
...
module.exports = { registrarError, limpiarContadores, msgErr, esOpenAISinCreditos, esSinNumeroVisible };
