...
function esPagoNeto(datos) {
  ...
  // `\bfavio\b` y no `/favio/` a secas: sin el borde de palabra matchea "Faviola", "Faviana" y
  // cualquier nombre que lo contenga. Mientras esto solo corría dentro de la ventana de 48h el
  // falso positivo era raro; desde el 14-ago-2026 corre para TODA imagen (es el contenido y ya
  // no el flag lo que decide si una captura es un comprobante), así que un yape de S/10 a una
  // Faviola pasaba a leerse como pago Pro. "Favio Mendoza" sigue matcheando.
  const destinoNeto = /\bneto\b/.test(comercio) || /\bfavio\b/.test(comercio);
...
/**
 * La línea de precios que va en TODO pitch de Pro por WhatsApp.
 *
 * Existía copiada como literal en 15 sitios de 8 archivos ("💰 *S/10/mes* o *S/99/año*"),
 * pese a que `PRO_PRECIOS` ya era la fuente única. Un cambio de precio obligaba a un barrido
 * a mano, y el que se olvidara un archivo le quedaba cotizando el precio viejo a un usuario
 * real — con la firma de que los sitios olvidados son siempre los intents menos transitados,
 * o sea los que nadie mira al verificar. Lo fija `tests/precios-unica-fuente.test.js`.
 */
function lineaPrecioPro() {
  return '💰 *S/' + PRO_PRECIOS.mensual + '/mes* o *S/' + PRO_PRECIOS.anual + '/año*';
}
...
module.exports = { validateConfig, ADMIN_NUMBER, PRO_PRECIOS, lineaPrecioPro, esPagoNeto, detectarTipoPlan, resolverTipoPlan, descuentoReferidoVigente, precioProEfectivo };
