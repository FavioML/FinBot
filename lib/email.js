...
/**
 * La dirección a la que se le puede escribir a este usuario, o null si no hay una PROBADA.
 *
 * `usuarios.email` no es una columna de direcciones probadas (15-sep-2026). Las filas nacidas en
 * WhatsApp pueden traer el correo que la persona DICTÓ en el alta viejo (paso 101, retirado el
 * 31-jul en `3c992bb`), y nadie lo verificó. Un error de tipeo que cae en la bandeja real de otra
 * persona le manda a un desconocido los avisos de plata de alguien más; en el d11/d14 del trial,
 * además, el link de activación firmado de su cuenta.
 *
 * La prueba que queda en pie es la cuenta web: la dirección de esas filas es la de su sesión de
 * Google o del magic link. Medido el 15-sep sobre producción: 111 de 113 filas con cuenta web
 * tienen `email` igual al de su usuario de auth, y las otras 2 lo tienen en null.
 *
 * **Lo que sostiene esa premisa, dicho sin exagerar.** `bindActivacion` dejó de conservar el
 * correo de la fila al adoptar (escribe el de la sesión), y la migración 086 borró los 21 dictados.
 * Pero TRES caminos siguen pudiendo pasarle a una fila web el correo que traía otra fila:
 * `merge_and_link` (`COALESCE(s.email, l.email)`), el fallback `otp.email || usuario.email` del OTP
 * (`handlers/webhook.js`, `services/otp-sin-numero.js`) y el reintento por 23505 de `bindActivacion`.
 * Hoy son inofensivos porque ningún escritor guarda una dirección sin probar, y eso NO lo vigila un
 * test: si un alta vuelve a pedir el correo fuera de una sesión, esos caminos lo llevan a una fila
 * que este helper lee como probada. Lo que queda es `qa-e2e/probe-email-no-verificado.mjs`, a mano.
 *
 * El corte vive acá y lo pone el LLAMADOR (`to: correoVerificado(usuario)`), no dentro de
 * `notificarUsuario`: el chokepoint no lee la base ni conoce la fila, y así se queda (ver el bloque
 * de `cuenta_borrada_at` en `lib/notify-user.js`). Por eso el select que trae al usuario tiene que
 * incluir `supabase_auth_id`: sin la columna esto devuelve null para todos y el canal se apaga en
 * silencio. Lo vigilan `tests/cron/email-necesita-su-columna.test.js` (la columna) y
 * `tests/notificaciones-duales.test.js` (que todo `to:` pase por acá).
 *
 * El precio, dicho: una fila sin cuenta web cuyo correo SÍ está probado por su propio Gmail (una
 * sola al 15-sep, de antes de que conectar Gmail fuera web-only) tampoco recibe correo.
 *
 * @param {{ supabase_auth_id?: string|null, email?: string|null }|null} usuario
 * @returns {string|null}
 */
function correoVerificado(usuario) {
  if (!usuario || !usuario.supabase_auth_id) return null;
  return usuario.email || null;
}

module.exports = {
  enviarEmail, correoVerificado, CANAL_EMAIL: CANAL,
